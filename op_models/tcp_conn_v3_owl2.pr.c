/* Process model C form file: tcp_conn_v3_owl2.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char tcp_conn_v3_owl2_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 554A8D72 554A8D72 1 OWL-1 Liliana 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                                            ";
#include <string.h>
...
typedef struct
	...
	int	                    		engine                                          ;
	int	                    		trans_init                                      ;
	int	                    		retransmission_ctr                              ;
	} tcp_conn_v3_owl2_state;
...
#define trans_init              		op_sv_ptr->trans_init
#define retransmission_ctr      		op_sv_ptr->retransmission_ctr
...
#define FIN_PREAMBLE_DEC	tcp_conn_v3_owl2_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((tcp_conn_v3_owl2_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
tcp_cwnd_update (TcpT_Size acked_bytes)
	...
	if (ev_ptr->flags & TCPC_FLAG_ECE)
		...
		if (cwnd < ssthresh)
			...
			// cwnd_pred = cwnd;
			printf("<%f><%d> cwnd = %d, cwnd_pred = %d, delta = %f, error = %f, var_control = %f, ai_var = %f, md_var = %f\n", op_sim_time(), my_id, (int) cwnd/snd_mss, cwnd_pred, Delta, error, var_control, ai_var, md_var);
...
// pred=(1.0-1.0/(float)cont_ser)*(float)cwnd_pred +(1.0/(float)cont_ser)*sqrt( 2*ai_var*(float) Delta /(1 - md_var*md_var) ); //sqrt(ai_var*(1+md_var)*(float) Delta/(2*(1-md_var)) ); /* sqrt(8.0*(float)Delta/3.0); */
	
				
if (design_on){
error= (float) ( (float) pred - (float) cwnd/ (float) snd_mss);   // cambiar a cwnd_pred

var_control = error/cwnd_pred;  //error*Kp+error_acum*Ki; // Derror*Kd+

ai_var=var_control;

if(ai_var<(float)0.03){
ai_var= (float)0.03;}
...
md_var=sqrt(1.0-(3.0/4.0)*ai_var);
}
...
tcp_timeout_retrans (void* PRG_ARG_UNUSED (input_ptr), int PRG_ARG_UNUSED (code))
	...
	// Modificaciones owl - zurita TIMEOUT - timeout_retras
...
if (design_on){	
	...
	vec_delta[cont_ser-1]=Delta;
	vec_cwnd_enfast[cont_ser-1]=cwnd/snd_mss;
	vec_tiempo2[cont_ser-1]=op_sim_time();
	...
	//Modificación usando promedio
	
	pred=(1.0-1.0/(float)cont_ser)*(float)cwnd_pred +(1.0/(float)cont_ser)*sqrt(3.0*(float)Delta/2.0); /* sqrt(8.0*(float)Delta/3.0); */
	...
	vec_cwnd_pred[cont_ser-1]=cwnd_pred;
...
if (design_on){	
	...
	Delta = (seg_ack-num_seq+snd_mss)/snd_mss;
	
	vec_delta[cont_ser-1]=Delta;
	vec_cwnd_enfast[cont_ser-1]=cwnd/snd_mss;
	vec_tiempo2[cont_ser-1]=op_sim_time();
	
	//pred=(1.0-1.0/(float)cont_ser)*(float)cwnd_pred +(1.0/(float)cont_ser)*sqrt( 2* (float)ai_var*(float) Delta /(1 - (float)md_var*(float)md_var) ); //sqrt(ai_var*(1+md_var)*(float) Delta/(2*(1-md_var)) ); /* sqrt(8.0*(float)Delta/3.0); */
	pred = 0.5*(float)cwnd_pred + 0.5*sqrt( 2* (float)ai_var*(float) Delta /(1 - (float)md_var*(float)md_var) ); //sqrt(ai_var*(1+md_var)*(float) Delta/(2*(1-md_var)) ); /* sqrt(8.0*(float)Delta/3.0); */
	
	error= (float) ( (float) pred - (float) cwnd/ (float) snd_mss);   // cambiar a cwnd_pred
	
	printf("<%f><%d> MD1 cwnd = %d, cwnd_pred = %d, pred = %f, delta = %f, error = %f, var_control = %f, ai_var = %f, md_var = %f\n", op_sim_time(), my_id, (int) cwnd/snd_mss, cwnd_pred, pred, Delta, error, var_control, ai_var, md_var);
	...
	//ssthresh = ((int) (cwnd*(float)md_var/(float)snd_mss)) * snd_mss;
	//	

	printf("<%f><%d> MD2 cwnd = %d, cwnd_pred = %d, pred = %f, delta = %f, error = %f, var_control = %f, ai_var = %f, md_var = %f, ssthresh = %d, (snd_nxt - snd_una) = %d, snd_mss = %d\n", op_sim_time(), my_id, (int) cwnd/snd_mss, cwnd_pred, pred, Delta, error, var_control, ai_var, md_var, ssthresh, (int) (snd_nxt - snd_una)/snd_mss, snd_mss); //(int)((float)(snd_nxt - snd_una)*(float)md_var), (int)((float)(snd_max - snd_una)*(float)md_var) );
	}
	// snd_max = %d, snd_nxt = %d, snd_una = %d, in-flight size = %d,                        snd_max, snd_nxt, snd_una, snd_max - snd_una, 
...
engEvalString(ep,"[ind,vel,n_iter]=prePso()");

ind=engGetVariable(ep,"ind");
ind_ptr=mxGetPr(ind);
memcpy(ind_data,ind_ptr,npart*nvar*sizeof(double));
mxDestroyArray(ind);

vel=engGetVariable(ep,"vel");
vel_ptr=mxGetPr(vel);
memcpy(vel_data,vel_ptr,npart*nvar*sizeof(double));
mxDestroyArray(vel);

n_iter=engGetVariable(ep,"n_iter");
...
extern "C" {
#endif
	void tcp_conn_v3_owl2 (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_tcp_conn_v3_owl2_init (int * init_block_ptr);
	void _op_tcp_conn_v3_owl2_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_tcp_conn_v3_owl2_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_tcp_conn_v3_owl2_alloc (VosT_Obtype, int);
	void _op_tcp_conn_v3_owl2_svar (void *, const char *, void **);
...
void
tcp_conn_v3_owl2 (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (tcp_conn_v3_owl2 ());
		...
		FSM_ENTER ("tcp_conn_v3_owl2")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "tcp_conn_v3_owl2 [init enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [init enter execs]", state0_enter_exec)
				{
				...
				var_control=0;
				error_ant=0;
				retransmission_ctr = 0;
				
				my_id = op_topo_parent (op_id_self());
				printf("server_0 id: %d\n", op_id_from_name (op_topo_parent (op_topo_parent (op_id_self ())), OPC_OBJTYPE_NDFIX, "server_0"));
				printf("server_1 id: %d\n", op_id_from_name (op_topo_parent (op_topo_parent (op_id_self ())), OPC_OBJTYPE_NDFIX, "server_1"));
				...
				if (OPC_COMPCODE_FAILURE==op_ima_obj_attr_get_dbl (my_id, "hola", &hola)){
				printf("Failure\n");}
				*/
				...
				*/
				...
				design_on = 1;
				printf("hola design es %d",design_on);
				design_nxt=1;
				engine=0;
				/*---------------------Algoritmo para PSO-------------------------------*/
				
				if (ISSERVER&&val_head==0&&engine){
				...
				for(i_z=0;i_z<=npart*nvar*2;++i_z){
				 ...
				 printf("fObj %f\n",ev_fobj);
				 ind=cortar_vec(p_result,0,npart*nvar-1);
				 vel=cortar_vec(p_result,npart*nvar,2*npart*nvar-1);
				 n_it=cortar_vec(p_result,2*npart*nvar,2*npart*nvar);
				 free(p_result);
				 printf(" vel ");
				...
				printf("Llamar post pso");
				 if(nRep[0]==10){
				mins=postPso(peso,ind,vel,testfunc,n_it[0]);}
			...
			FSM_STATE_EXIT_FORCED (0, "init", "tcp_conn_v3_owl2 [init exit execs]")


			/** state (init) transition processing **/
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [init trans conditions]", state0_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "OPEN_PASSIVE", "", "init", "LISTEN", "tr_1", "tcp_conn_v3_owl2 [init -> LISTEN : OPEN_PASSIVE / ]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, SEND_SYN;, "OPEN_ACTIVE", "SEND_SYN", "init", "SYN_SENT", "tr_2", "tcp_conn_v3_owl2 [init -> SYN_SENT : OPEN_ACTIVE / SEND_SYN]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (1, "LISTEN", state1_enter_exec, "tcp_conn_v3_owl2 [LISTEN enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [LISTEN enter execs]", state1_enter_exec)
				{
			...
			FSM_EXIT (3,"tcp_conn_v3_owl2")


			/** state (LISTEN) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "LISTEN", "tcp_conn_v3_owl2 [LISTEN exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [LISTEN exit execs]", state1_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [LISTEN trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, SEND_SYN_ACK;, "RCV_SYN", "SEND_SYN_ACK", "LISTEN", "SYN_RCVD", "tr_3", "tcp_conn_v3_owl2 [LISTEN -> SYN_RCVD : RCV_SYN / SEND_SYN_ACK]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, SEND_SYN;, "SEND", "SEND_SYN", "LISTEN", "SYN_SENT", "tr_4", "tcp_conn_v3_owl2 [LISTEN -> SYN_SENT : SEND / SEND_SYN]")
				FSM_CASE_TRANSIT (2, 11, state11_enter_exec, ;, "CLOSE", "", "LISTEN", "CLOSED", "tr_23", "tcp_conn_v3_owl2 [LISTEN -> CLOSED : CLOSE / ]")
				FSM_CASE_TRANSIT (3, 3, state3_enter_exec, SEND_SYN;, "OPEN_ACTIVE", "SEND_SYN", "LISTEN", "SYN_SENT", "tr_34", "tcp_conn_v3_owl2 [LISTEN -> SYN_SENT : OPEN_ACTIVE / SEND_SYN]")
				FSM_CASE_TRANSIT (4, 1, state1_enter_exec, ;, "default", "", "LISTEN", "LISTEN", "tr_24", "tcp_conn_v3_owl2 [LISTEN -> LISTEN : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (2, "SYN_RCVD", state2_enter_exec, "tcp_conn_v3_owl2 [SYN_RCVD enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [SYN_RCVD enter execs]", state2_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"tcp_conn_v3_owl2")


			/** state (SYN_RCVD) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "SYN_RCVD", "tcp_conn_v3_owl2 [SYN_RCVD exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [SYN_RCVD exit execs]", state2_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [SYN_RCVD trans conditions]", state2_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 4, state4_enter_exec, SEND_ACK;, "RCV_ACK_OF_SYN", "SEND_ACK", "SYN_RCVD", "ESTAB", "tr_6", "tcp_conn_v3_owl2 [SYN_RCVD -> ESTAB : RCV_ACK_OF_SYN / SEND_ACK]")
				FSM_CASE_TRANSIT (1, 5, state5_enter_exec, RSND_SYN_FIN;, "CLOSE", "RSND_SYN_FIN", "SYN_RCVD", "FINWAIT1", "tr_8", "tcp_conn_v3_owl2 [SYN_RCVD -> FINWAIT1 : CLOSE / RSND_SYN_FIN]")
				FSM_CASE_TRANSIT (2, 1, state1_enter_exec, ;, "PASSIVE_RESET", "", "SYN_RCVD", "LISTEN", "tr_35", "tcp_conn_v3_owl2 [SYN_RCVD -> LISTEN : PASSIVE_RESET / ]")
				FSM_CASE_TRANSIT (3, 9, state9_enter_exec, RSND_SYN_ACK;, "RCV_FIN", "RSND_SYN_ACK", "SYN_RCVD", "CLOSE_WAIT", "tr_36", "tcp_conn_v3_owl2 [SYN_RCVD -> CLOSE_WAIT : RCV_FIN / RSND_SYN_ACK]")
				FSM_CASE_TRANSIT (4, 9, state9_enter_exec, SEND_ACK;, "RCV_FIN_ACK_OF_SYN", "SEND_ACK", "SYN_RCVD", "CLOSE_WAIT", "tr_82", "tcp_conn_v3_owl2 [SYN_RCVD -> CLOSE_WAIT : RCV_FIN_ACK_OF_SYN / SEND_ACK]")
				FSM_CASE_TRANSIT (5, 2, state2_enter_exec, ;, "default", "", "SYN_RCVD", "SYN_RCVD", "tr_26", "tcp_conn_v3_owl2 [SYN_RCVD -> SYN_RCVD : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (3, "SYN_SENT", state3_enter_exec, "tcp_conn_v3_owl2 [SYN_SENT enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [SYN_SENT enter execs]", state3_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (7,"tcp_conn_v3_owl2")


			/** state (SYN_SENT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (3, "SYN_SENT", "tcp_conn_v3_owl2 [SYN_SENT exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [SYN_SENT exit execs]", state3_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [SYN_SENT trans conditions]", state3_trans_conds)
			FSM_INIT_COND (RCV_SYN)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, RSND_SYN_ACK;, "RCV_SYN", "RSND_SYN_ACK", "SYN_SENT", "SYN_RCVD", "tr_5", "tcp_conn_v3_owl2 [SYN_SENT -> SYN_RCVD : RCV_SYN / RSND_SYN_ACK]")
				FSM_CASE_TRANSIT (1, 4, state4_enter_exec, SEND_ACK;, "RCV_SYN_ACK", "SEND_ACK", "SYN_SENT", "ESTAB", "tr_7", "tcp_conn_v3_owl2 [SYN_SENT -> ESTAB : RCV_SYN_ACK / SEND_ACK]")
				FSM_CASE_TRANSIT (2, 11, state11_enter_exec, ;, "CLOSE", "", "SYN_SENT", "CLOSED", "tr_22", "tcp_conn_v3_owl2 [SYN_SENT -> CLOSED : CLOSE / ]")
				FSM_CASE_TRANSIT (3, 9, state9_enter_exec, SEND_ACK;, "RCV_FIN_ACK_OF_SYN", "SEND_ACK", "SYN_SENT", "CLOSE_WAIT", "tr_71", "tcp_conn_v3_owl2 [SYN_SENT -> CLOSE_WAIT : RCV_FIN_ACK_OF_SYN / SEND_ACK]")
				FSM_CASE_TRANSIT (4, 3, state3_enter_exec, ;, "default", "", "SYN_SENT", "SYN_SENT", "tr_25", "tcp_conn_v3_owl2 [SYN_SENT -> SYN_SENT : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (4, "ESTAB", state4_enter_exec, "tcp_conn_v3_owl2 [ESTAB enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [ESTAB enter execs]", state4_enter_exec)
				{
				tcp_state_enter ("ESTAB");
				
				if (trans_init){
				
				printf("<%f> Connection Established\n", op_sim_time() );
				printf("<%f><%d> CE1 cwnd = %d, cwnd_pred = %d, delta = %f, error = %f, var_control = %f, ai_var = %f, md_var = %f\n", op_sim_time(), my_id, (int) cwnd/snd_mss, cwnd_pred, Delta, error, var_control, ai_var, md_var);
				ai_var = 1;
				md_var = (float) 0.5;
				cont_ser=1;
				cwnd_pred = 100;
				printf("<%f><%d> CE2 cwnd = %d, cwnd_pred = %d, delta = %f, error = %f, var_control = %f, ai_var = %f, md_var = %f\n", op_sim_time(), my_id, (int) cwnd/snd_mss, cwnd_pred, Delta, error, var_control, ai_var, md_var);
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (9,"tcp_conn_v3_owl2")


			/** state (ESTAB) exit executives **/
			FSM_STATE_EXIT_UNFORCED (4, "ESTAB", "tcp_conn_v3_owl2 [ESTAB exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [ESTAB exit execs]", state4_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [ESTAB trans conditions]", state4_trans_conds)
			FSM_INIT_COND (CLOSE)
			FSM_TEST_COND (RCV_FIN)
			FSM_DFLT_COND
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 5, state5_enter_exec, SEND_FIN;, "CLOSE", "SEND_FIN", "ESTAB", "FINWAIT1", "tr_9", "tcp_conn_v3_owl2 [ESTAB -> FINWAIT1 : CLOSE / SEND_FIN]")
				FSM_CASE_TRANSIT (1, 9, state9_enter_exec, SEND_ACK;, "RCV_FIN", "SEND_ACK", "ESTAB", "CLOSE_WAIT", "tr_11", "tcp_conn_v3_owl2 [ESTAB -> CLOSE_WAIT : RCV_FIN / SEND_ACK]")
				FSM_CASE_TRANSIT (2, 4, state4_enter_exec, ;, "default", "", "ESTAB", "ESTAB", "tr_33", "tcp_conn_v3_owl2 [ESTAB -> ESTAB : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (5, "FINWAIT1", state5_enter_exec, "tcp_conn_v3_owl2 [FINWAIT1 enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [FINWAIT1 enter execs]", state5_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (11,"tcp_conn_v3_owl2")


			/** state (FINWAIT1) exit executives **/
			FSM_STATE_EXIT_UNFORCED (5, "FINWAIT1", "tcp_conn_v3_owl2 [FINWAIT1 exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [FINWAIT1 exit execs]", state5_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [FINWAIT1 trans conditions]", state5_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 8, state8_enter_exec, SEND_ACK;, "RCV_FIN", "SEND_ACK", "FINWAIT1", "CLOSING", "tr_12", "tcp_conn_v3_owl2 [FINWAIT1 -> CLOSING : RCV_FIN / SEND_ACK]")
				FSM_CASE_TRANSIT (1, 6, state6_enter_exec, ;, "RCV_ACK_OF_FIN", "", "FINWAIT1", "FINWAIT2", "tr_13", "tcp_conn_v3_owl2 [FINWAIT1 -> FINWAIT2 : RCV_ACK_OF_FIN / ]")
				FSM_CASE_TRANSIT (2, 7, state7_enter_exec, SEND_ACK;, "RCV_FIN_ACK_OF_FIN", "SEND_ACK", "FINWAIT1", "TIME_WAIT", "tr_40", "tcp_conn_v3_owl2 [FINWAIT1 -> TIME_WAIT : RCV_FIN_ACK_OF_FIN / SEND_ACK]")
				FSM_CASE_TRANSIT (3, 5, state5_enter_exec, ;, "default", "", "FINWAIT1", "FINWAIT1", "tr_27", "tcp_conn_v3_owl2 [FINWAIT1 -> FINWAIT1 : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (6, "FINWAIT2", state6_enter_exec, "tcp_conn_v3_owl2 [FINWAIT2 enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [FINWAIT2 enter execs]", state6_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (13,"tcp_conn_v3_owl2")


			/** state (FINWAIT2) exit executives **/
			FSM_STATE_EXIT_UNFORCED (6, "FINWAIT2", "tcp_conn_v3_owl2 [FINWAIT2 exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [FINWAIT2 exit execs]", state6_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [FINWAIT2 trans conditions]", state6_trans_conds)
			FSM_INIT_COND (RCV_FIN)
			FSM_DFLT_COND
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 7, state7_enter_exec, SEND_ACK;, "RCV_FIN", "SEND_ACK", "FINWAIT2", "TIME_WAIT", "tr_14", "tcp_conn_v3_owl2 [FINWAIT2 -> TIME_WAIT : RCV_FIN / SEND_ACK]")
				FSM_CASE_TRANSIT (1, 6, state6_enter_exec, ;, "default", "", "FINWAIT2", "FINWAIT2", "tr_28", "tcp_conn_v3_owl2 [FINWAIT2 -> FINWAIT2 : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (7, "TIME_WAIT", state7_enter_exec, "tcp_conn_v3_owl2 [TIME_WAIT enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [TIME_WAIT enter execs]", state7_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (15,"tcp_conn_v3_owl2")


			/** state (TIME_WAIT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (7, "TIME_WAIT", "tcp_conn_v3_owl2 [TIME_WAIT exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [TIME_WAIT exit execs]", state7_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [TIME_WAIT trans conditions]", state7_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 11, state11_enter_exec, ;, "TIMEOUT", "", "TIME_WAIT", "CLOSED", "tr_15", "tcp_conn_v3_owl2 [TIME_WAIT -> CLOSED : TIMEOUT / ]")
				FSM_CASE_TRANSIT (1, 7, state7_enter_exec, ;, "default", "", "TIME_WAIT", "TIME_WAIT", "tr_31", "tcp_conn_v3_owl2 [TIME_WAIT -> TIME_WAIT : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (8, "CLOSING", state8_enter_exec, "tcp_conn_v3_owl2 [CLOSING enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [CLOSING enter execs]", state8_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (17,"tcp_conn_v3_owl2")


			/** state (CLOSING) exit executives **/
			FSM_STATE_EXIT_UNFORCED (8, "CLOSING", "tcp_conn_v3_owl2 [CLOSING exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [CLOSING exit execs]", state8_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [CLOSING trans conditions]", state8_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 7, state7_enter_exec, ;, "RCV_ACK_OF_FIN", "", "CLOSING", "TIME_WAIT", "tr_17", "tcp_conn_v3_owl2 [CLOSING -> TIME_WAIT : RCV_ACK_OF_FIN / ]")
				FSM_CASE_TRANSIT (1, 8, state8_enter_exec, ;, "default", "", "CLOSING", "CLOSING", "tr_30", "tcp_conn_v3_owl2 [CLOSING -> CLOSING : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (9, "CLOSE_WAIT", state9_enter_exec, "tcp_conn_v3_owl2 [CLOSE_WAIT enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [CLOSE_WAIT enter execs]", state9_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (19,"tcp_conn_v3_owl2")


			/** state (CLOSE_WAIT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (9, "CLOSE_WAIT", "tcp_conn_v3_owl2 [CLOSE_WAIT exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [CLOSE_WAIT exit execs]", state9_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [CLOSE_WAIT trans conditions]", state9_trans_conds)
			FSM_INIT_COND (CLOSE)
			FSM_DFLT_COND
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 10, state10_enter_exec, SEND_FIN;, "CLOSE", "SEND_FIN", "CLOSE_WAIT", "LAST_ACK", "tr_19", "tcp_conn_v3_owl2 [CLOSE_WAIT -> LAST_ACK : CLOSE / SEND_FIN]")
				FSM_CASE_TRANSIT (1, 9, state9_enter_exec, ;, "default", "", "CLOSE_WAIT", "CLOSE_WAIT", "tr_29", "tcp_conn_v3_owl2 [CLOSE_WAIT -> CLOSE_WAIT : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (10, "LAST_ACK", state10_enter_exec, "tcp_conn_v3_owl2 [LAST_ACK enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [LAST_ACK enter execs]", state10_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (21,"tcp_conn_v3_owl2")


			/** state (LAST_ACK) exit executives **/
			FSM_STATE_EXIT_UNFORCED (10, "LAST_ACK", "tcp_conn_v3_owl2 [LAST_ACK exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [LAST_ACK exit execs]", state10_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [LAST_ACK trans conditions]", state10_trans_conds)
			FSM_INIT_COND (RCV_ACK_OF_FIN)
			FSM_DFLT_COND
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 11, state11_enter_exec, ;, "RCV_ACK_OF_FIN", "", "LAST_ACK", "CLOSED", "tr_18", "tcp_conn_v3_owl2 [LAST_ACK -> CLOSED : RCV_ACK_OF_FIN / ]")
				FSM_CASE_TRANSIT (1, 10, state10_enter_exec, ;, "default", "", "LAST_ACK", "LAST_ACK", "tr_32", "tcp_conn_v3_owl2 [LAST_ACK -> LAST_ACK : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (11, "CLOSED", state11_enter_exec, "tcp_conn_v3_owl2 [CLOSED enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_owl2 [CLOSED enter execs]", state11_enter_exec)
				{
				//modificaciones
				...
				printf("<%f><%d> Retransmission counter: %d\n", op_sim_time(), my_id, retransmission_ctr);
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (23,"tcp_conn_v3_owl2")


			/** state (CLOSED) exit executives **/
			FSM_STATE_EXIT_UNFORCED (11, "CLOSED", "tcp_conn_v3_owl2 [CLOSED exit execs]")
...
void
_op_tcp_conn_v3_owl2_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	...
	FIN_MT (_op_tcp_conn_v3_owl2_diag ())
...
void
_op_tcp_conn_v3_owl2_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = __LINE__;
#endif

	FIN_MT (_op_tcp_conn_v3_owl2_terminate ())
...
/* local variable prs_ptr in _op_tcp_conn_v3_owl2_svar function. */
#undef tcb_ptr
...
VosT_Obtype
_op_tcp_conn_v3_owl2_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_tcp_conn_v3_owl2_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (tcp_conn_v3_owl2)",
		sizeof (tcp_conn_v3_owl2_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_tcp_conn_v3_owl2_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	tcp_conn_v3_owl2_state * ptr;
	FIN_MT (_op_tcp_conn_v3_owl2_alloc (obtype))

	ptr = (tcp_conn_v3_owl2_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "tcp_conn_v3_owl2 [init enter execs]";
#endif
		}
...
void
_op_tcp_conn_v3_owl2_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	tcp_conn_v3_owl2_state		*prs_ptr;

	FIN_MT (_op_tcp_conn_v3_owl2_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (tcp_conn_v3_owl2_state *)gen_ptr;
	...
	if (strcmp ("trans_init" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->trans_init);
		FOUT
		}
	if (strcmp ("retransmission_ctr" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->retransmission_ctr);
		FOUT
		}
	*var_p_ptr = (void *)OPC_NIL;
